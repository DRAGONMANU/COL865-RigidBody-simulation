...
void drawBox(vec3 xmin, vec3 xmax, bool drawSurf=true, bool drawWire=true);
void drawSphere(vec3 center, float radius, bool drawSurf=true, bool drawWire=true);

void translate(vec3 x);
void rotate(quat q);
void pushTransform();
void popTransform();
...
void drawBox(vec3 xmin, vec3 xmax, bool drawSurf, bool drawWire) {
    ...
    if (drawWire) {
        ...
        drawLine(vec3(x0,y0,z0), vec3(x1,y0,z0));
        drawLine(vec3(x0,y1,z0), vec3(x1,y1,z0));
        drawLine(vec3(x0,y0,z1), vec3(x1,y0,z1));
        drawLine(vec3(x0,y1,z1), vec3(x1,y1,z1));
        drawLine(vec3(x0,y0,z0), vec3(x0,y1,z0));
        drawLine(vec3(x1,y0,z0), vec3(x1,y1,z0));
        drawLine(vec3(x0,y0,z1), vec3(x0,y1,z1));
        drawLine(vec3(x1,y0,z1), vec3(x1,y1,z1));
        drawLine(vec3(x0,y0,z0), vec3(x0,y0,z1));
        drawLine(vec3(x1,y0,z0), vec3(x1,y0,z1));
        drawLine(vec3(x0,y1,z0), vec3(x0,y1,z1));
        drawLine(vec3(x1,y1,z0), vec3(x1,y1,z1));
        glPopAttrib();
        if (drawSurf)
            glPopAttrib();
    }
...
void drawCircle(vec3 center, vec3 axis0, vec3 axis1) {
    int n = 60;
    glBegin(GL_LINE_LOOP);
    for (int i = 0; i < n; i++) {
        vec3 x = center + axis0*cos(2*M_PI*i/n) + axis1*sin(2*M_PI*i/n);
        glVertex3f(x[0], x[1], x[2]);
    }
    glEnd();
}

void drawSphere(vec3 center, float radius, bool drawSurf, bool drawWire) {
    glPushMatrix();
    ...
    if (drawWire) {
        ...
        glPushAttrib(GL_ENABLE_BIT);
        glDisable(GL_LIGHTING);
        drawCircle(vec3(0,0,0),radius*vec3(1,0,0),radius*vec3(0,1,0));
        drawCircle(vec3(0,0,0),radius*vec3(0,1,0),radius*vec3(0,0,1));
        drawCircle(vec3(0,0,0),radius*vec3(0,0,1),radius*vec3(1,0,0));
        glPopAttrib();
        if (drawSurf)
            glPopAttrib();
    }
